...
\begin{document}
    ...
    \begin{Objective}{Linearize Equations of Motion}
        \begin{enumerate}[(a)]
            ...
            Now, we define:
            \[
                \tilde{z}_v \equiv z_v - z_v^{(e)},\ \tilde{h} \equiv h - h_e,\ \tilde{\theta} \equiv \theta - \theta_e,\ \tilde F \equiv F - F_e,\ \tilde \tau \equiv \tau - \tau_e = \tau
            \]

            Also, we can note that as $\mathbf{\dot q}_e = 0$ and $\mathbf{\ddot q}_e = 0$, so we can write:
            \[\dot{\tilde z}_v = \dot z_v,\ \ddot{\tilde z}_v = \ddot z_v\]
            ...
            \[
                \begin{split}
                    \frac{\mu}{M_V}\dot z_v & \approx \frac{\mu}{M_V}\dot z_v^{(e)} + \pde{}{\dot z_v}(\frac{\mu}{M_V}\dot z_v) \bigg\rvert_{eq.} (\dot z_v - \dot z_v^{(e)}) \\
                    & = \frac{\mu}{M_V}\dot{\tilde z}_v \\
                    \frac{F}{M_V} \sin\theta & \approx \frac{F}{M_V} \sin\theta_e + \pde{}{\theta}(\frac{F}{M_V} \sin\theta) \bigg\rvert_{eq.} (\theta - \theta_e) \\
                    & = \frac{F_e}{M_V} \tilde\theta\cos\theta_e \\
                    & = g \tilde\theta \\
                    \frac{F}{M_V} \cos\theta & \approx \frac{F}{M_V} \cos\theta_e + \pde{}{\theta}(\frac{F}{M_V} \cos\theta) \bigg\rvert_{eq.} (\theta - \theta_e) \\
            ...
            \begin{equation}
                ...
                \begin{split}
                    \ddot{\tilde z}_v & = - \frac{\mu}{M_V}\dot{\tilde z}_v - g\tilde\theta \\
